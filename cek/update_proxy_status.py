...
def main():
    ...
    with ThreadPoolExecutor(max_workers=50) as executor:
        futures = [executor.submit(check_proxy, row, api_url_template, output_file) for row in rows if len(row) >= 2]
        for future in as_completed(futures):
            future.result()  # Tunggu setiap proses selesai

    print(f"Semua proxy yang ALIVE telah disimpan di {output_file}.")
